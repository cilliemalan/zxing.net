...
namespace ZXing.PDF417.Internal
{
    ...
    public sealed class Detector
    {
        ...
        private const double rotationThresh = 0.017453292519943295;
        ...
        public static PDF417DetectorResult detect(BinaryBitmap image, IDictionary<DecodeHintType, object> hints, bool multiple)
        {
            ...
            if (!multiple)
            {
                bitMatrix = AutoRotateIfNeeded(bitMatrix);
            }
        ...
        private static BitMatrix AutoRotateIfNeeded(BitMatrix src)
        {
            //find initial vertices
            ResultPoint[] vertices = findVertices(src, 0, 0);

            //4 -> 2
            //1 -> 7
            if (vertices != null && vertices[4] != null && vertices[2] != null && vertices[1] != null && vertices[7] != null)
            {
                var a1 = GetAngle(vertices[4], vertices[2]);
                var a2 = GetAngle(vertices[1], vertices[7]);
        ...
        private static double GetAngle(ResultPoint p1, ResultPoint p2)
        {
            float dy = p2.Y - p1.Y;
            float dx = p2.X - p1.X;
           ...
           int[] pattern)
        {
            ...
            while (startRow < height)
            {
                ...
                for (; startRow < height; startRow += ROW_STEP)
                {
                    ...
                    if (loc != null)
                    {
                        int stopper = Math.Max(0, startRow - ROW_STEP + 1);
                        while (startRow > stopper)
                        {
